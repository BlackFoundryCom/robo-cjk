...
class DesignFrameDrawer():
    ...
    def draw(self, 
        ...
        stroke(rc, gc, bc, ac)
        x, y = 0, 0
        w = self.c.project.settings['designFrame']['em_Dimension'][0]
        h = self.c.project.settings['designFrame']['em_Dimension'][1]
        translateY = -12 * h / 100
        translate(0,translateY)
        if mainFrames:
            rect(x, y, w, h)

            x, y, charfaceW, charfaceH = self.getEmRatioFrame(self.c.project.settings['designFrame']['characterFace'], w, h)
            rect(x, y, charfaceW, charfaceH)

            stroke(None)
            fill(0,.75,1,.3)
            outside, inside = self.c.project.settings['designFrame']['overshoot'][1], self.c.project.settings['designFrame']['overshoot'][0]
        ...
        if secondLines:
            fill(None)
            stroke(.65, 0.16, .39, 1)
            ratio = (h * .5 * (self.c.project.settings['designFrame']['horizontalLine'] / 50))
            y = h * .5 - ratio
            height = h * .5 + ratio
            self.makeHorSecLine(RGlyph(), 0, y+translate_secondLine_Y, w, height+translate_secondLine_Y)
            ratio = (w * .5 * (self.c.project.settings['designFrame']['verticalLine'] / 50))
            x = w * .5 - ratio
            width = w * .5 + ratio
        ...
        if customsFrames:
            ...
            rc, gc, bc, ac = blackColor
            stroke(rc, gc, bc, ac)
            for frame in self.c.project.settings['designFrame']['customsFrames']:
                if not "Value" in frame: continue
                x, y, charfaceW, charfaceH = self.getEmRatioFrame(frame["Value"], w, h)
                rect(x, y, charfaceW, charfaceH)
        restore()